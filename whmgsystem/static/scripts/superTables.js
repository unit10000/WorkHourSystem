...
//设置table大小
function setScreenSize(height,width){
            $("#div_container").css("width",width);//这个宽度是容器宽度，不同容器宽度不同
            $(".fakeContainer").css("height",height);//这个高度是整个table可视区域的高度，不同情况高度不同
            first_td_width = $("#my_tr").children().eq(0).outerWidth()+$("#my_tr").children().eq(1).outerWidth()
            div_width = $("#div_container").width()-first_td_width

            div_height = $(".fakeContainer").height()-$("#my_tr").children().eq(0).outerHeight()
            //.sData是调用superTables.js之后页面自己生成的  这块就是出现滚动条 达成锁定表头和列的效果
            $(".sData").css("width",div_width+"px");//这块的宽度是用$("#div_container")的宽度减去锁定的列的宽度
            $(".sData").css("height",div_height+"px");//这块的高度是用$("#div_container")的高度减去锁定的表头的高度

            var X = $('#div_container').offset().top;
            var Y = $('#div_container').offset().left;
            $("#div_container").css("position","relative")
            $("#tableScreen_button").css("position","absolute")
            $("#tableScreen_button").css("left",10)
            $("#tableScreen_button").css("top",div_height+10)
            $("#tableScreen_button").css("z-index",1)
}
//获取法定节假日
function getWeekend(date_array){
//    &("#my_tr")
     $.post("/api/get_jiari",
      {
        days:date_array
...
//星期天加红
function setWeekendToTable(){
    td_objs = $(".my_title").children()

//    if (td_objs.length-2!=array_obj.length){
//        alert(array_obj.length)
//        return
//    }
    for (var i = 2 ; i <  td_objs.length ; i++){
        obj = td_objs.eq(i)
        if (obj.html().indexOf('日')>-1 || obj.html().indexOf('六')>-1){
            obj.css("color","red")
        }
    }
}
